...
def length_count(csv_file, path):
    ...
    The function returns a dictionary with sources as key values and average lengths as values"""
    ...
    for filename in os.listdir(os.getcwd()):
        if filename.endswith('_s.txt'):
            source = data[data['Статья'] == filename]['источник'].to_string(index=False).strip(' ')
...
if __name__ == "__main__":
    result = length_count('Practice.csv', os.getcwd()+'\praktika')
    print(result)
